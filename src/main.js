...
function resetGame() {
  ...
  setOverlay("Difese pronte", "Spendi punti costruzione, poi avvia la prossima ondata.");
  updateUi();
}
...
function finishWave() {
  ...
  state.cities.forEach((city) => {
    if (city.hp > 0) {
      city.ammo = Math.min(24 + city.weaponLevel * 4, city.ammo + 6 + city.factory);
...
function renderCities() {
  ui.cities.innerHTML = state.cities
    .map((city, index) => {
      ...
      const weapon = city.weapon === "launcher" ? "Lanciamissili" : turretDefs[city.weapon]?.label || "Nessuna";
      return `
...
function spendUpgrade(kind) {
  ...
  if (kind === "repair") {
    ...
    if (city.weapon === newWeapon) {
      city.weaponLevel = Math.min(5, city.weaponLevel + 1);
    } else {
      city.weapon = newWeapon;
      city.weaponLevel = 1;
      city.cooldown = 0;
      city.heat = 0;
    }
...
function launchMissile(target, type, byAi = false) {
  ...
  const launchers = state.cities.filter((city) => city.hp > 0 && city.weapon === "launcher" && city.disabled <= 0);
  ...
  const unlocked = city.weaponLevel >= def.unlock || type === "standard";
...
function fireTurret(city, type, dt, byAi = false) {
  if (!city || city.hp <= 0 || city.disabled > 0 || city.weapon === "launcher") return;
  ...
  if (type === "laser") {
    ...
    if (target) {
      target.hp -= def.damage * dt * (1 + city.weaponLevel * 0.22);
    ...
    state.friendlyBullets.push({
      ...
      damage: def.damage * (1 + city.weaponLevel * 0.18),
      life: 120,
  ...
  city.heat += def.heat;
  city.cooldown = Math.max(40, def.cooldown - city.weaponLevel * 28);
...
function autoTurrets(dt) {
  state.cities.forEach((city) => {
    if (city.weapon === "launcher" || city.hp <= 0) return;
    ...
    fireTurret(city, city.weapon, dt, true);
  });
}
...
function updatePlayerTurret(dt) {
  ...
  if (!city || city.weapon === "launcher") return;
...
function drawCrosshair() {
  ...
  if (!city || city.weapon === "launcher" || city.hp <= 0) return;
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key >= "1" && event.key <= "5") {
    state.selectedMissile = Object.keys(missileDefs)[Number(event.key) - 1];
